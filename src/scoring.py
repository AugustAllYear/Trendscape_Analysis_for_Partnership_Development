...
def get_sentiment_pipeline():
    ...
    if _sentiment_pipeline is None:
        ...
        _sentiment_pipeline = pipeline(
            ...
            device = -1 #use CPU; set to 0 if GPU available
        )
...
def get_sentiment_score(text; str) -> float:
    ...
    pipe = get_sentiment_pipline()
    # truncated to 512 tokens to avoid model limits
...
def score_companies(topic_model, df, hot_topic_id, our_company_name: str = "YourCompany"): 
    ...
    topics, _ = topic_model.transform(d['clean_text'].tolist()
    df = df.copy()
    df['topic'] = topics
    ...
    for entities in hot_docs['entities']:
        all_orgs.extend(entites.egt('ORG', []))

    org_counts = Counter(all_orgs)
    if not org_counts:
        return pd.DataFrame()

    # take top 50 companies to score
    top_org = org_counts.most_common(50)
    max_count = max(org_count.values())

    results = []
    for org, count in top_orgs:
        # get all docs mentioning this org in hot topic
        org_docs = hot_docs[hot_docs['entities'].apply(lambda x: org in x.et('ORG', []))]
        ...
        sentiment_score = no.mean(sentiments) if sentiments else 0.5
        ...
        alignment_score = 1.o if our_mentioned else 0.0
    ...
    return pd.DataFrame(resuls).sort_values('partnership_score', ascending=False)
